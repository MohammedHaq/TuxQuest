...
/**
 *
 *@param type The area type in which the player is in currently is taken in as a parameter 
 *@param *tux An instanct of the player structure is given as a parameter and stores the players current health,max health, XP, Gold, Level, ATK.
 *@return The function returns a bool value. if the function were to return true this indicates that the game has been completed. However, if the function does not return true then the game will continue
 *
 */
...
bool runEncounter (enum Area type, Player *tux) {
	...
	while (tux->stats.HP > 0 && creature.HP > 0) {
	  bool player_turn = true;
	  bool creature_turn = false;
	  int damage; 
	  if (player_turn == true) {
	    damage = 0;
	    damage = tux->stats.ATK - creature.DEF;
	  ...
	  if (creature_turn == true) {
	    damage = 0;
	    damage = creature.ATK - tux->stats.DEF;
	    printf("The %s attacks! %d damage!", creature.NAME, damage);
	    tux->stats.HP = tux->stats.HP - damage;
	    if (tux->stats.HP <= 0){
	      break;
	    }
	    damage = 0;
	...
	if (tux ->stats.HP <= 0){
	  ...
	  XP = creature.XP;
	  tux->stats.XP = tux->stats.XP + XP;
	  printf("Gained %d XP\n",XP);
	  Gold = XP / 2;
	  tux->gold = tux->gold + Gold;
	  printf("Gained %d Gold\n", Gold);
	  while (tux->stats.XP > 100){
	    tux->level = tux->level + 1;
	    tux->stats.MAX_HP = tux->stats.MAX_HP + 5;
	    tux->stats.HP = tux->stats.MAX_HP;
	    tux->stats.ATK = tux->stats.ATK + 1;
	    tux->stats.XP = tux->stats.XP - 100;
	  }
